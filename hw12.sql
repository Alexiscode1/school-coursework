...
CREATE TABLE long as
  select name from tracks where milliseconds>360000;
...
CREATE TABLE long_album as
  select a.title from albums a, tracks t where t.milliseconds>360000 and a.albumId = t.albumId;
...
CREATE TABLE album_count as
  select name, count(*) from artists ar ,albums a where a.artistId = ar.artistId group by ar.name;


-- Question 8
CREATE TABLE busiest_artists as
  select ar.name, count(t.albumId) from tracks t, albums a, artists ar where t.albumId = a.albumId and ar.artistId=a.artistId group by ar.name;
