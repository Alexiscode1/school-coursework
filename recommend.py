...
def find_closest(location, centroids):
    ...
    """
    # BEGIN Question 3
    if len(centroids)==1: #base case if only one centroid
        return centroids[0] 
    else:
        minimum = min([distance(location,centroid) for centroid in centroids])
        for i in range(len(centroids)):
            if distance(location, centroids[i])==minimum:
                return centroids[i]
...
def group_by_centroid(restaurants, centroids):
    ...
    for i in restaurants:
        ...
        pairs.append([centroid, i])
    return group_by_first(pairs) #same centroid putting into same cluster
    # END Question 4
...
def k_means(restaurants, k, max_updates=100):
    ...
    # randomly choosing k different centroids
    centroids = [restaurant_location(r) for r in sample(restaurants, k)]

    while previous_centroids != centroids and n < max_updates:
        previous_centroids = centroids
        # BEGIN Question 6
        clusters = group_by_centroid(restaurants, previous_centroids) # k clusters
        centroids =[find_centroid(clusters[i]) for i in range(len(clusters))] # finding new centroids after we have k clusters
        # END Question 6
        n += 1
    return centroids
...
def find_predictor(user, restaurants, feature_fn):
    ...
    lst1 = [x-averagex for x in xs]
    lst2 = [y-averagey for y in ys]

    Sxx=sum([(x-averagex)**2 for x in xs])
    Syy = sum([(y-averagey)**2 for y in ys])
    Sxy = sum([a * b for a, b in zip(lst1, lst2)])
...
def best_predictor(user, restaurants, feature_fns):
    ...
    """
    reviewed = user_reviewed_restaurants(user, restaurants) #Return all subsets of restaurants reviewed by user
    # BEGIN Question 8
    pairs_lst = []
    for feature in feature_fns:
        pair = list(find_predictor(user, reviewed, feature)) # a list of [predictor, r_squared] pairs
        pairs_lst.append(pair)
    maxR = max([i[1] for i in pairs_lst])
    for i in pairs_lst:
        if i[1] ==maxR:
            return i[0]
    # END Question 8
...
def rate_all(user, restaurants, feature_fns):
    ...
    """
    predictor = best_predictor(user, ALL_RESTAURANTS, feature_fns) # predictor is a function
    reviewed = user_reviewed_restaurants(user, restaurants) #the restaurants reviewed by the user
    # BEGIN Question 9
    dic = {}
    for i in range(len(restaurants)):
        name = restaurant_name(restaurants[i])
        if restaurants[i] in reviewed:
            score =  user_score(user, name) #Return the score given for restaurant_name by user.
            dic[name] = score # adding new item to the dictionary
        else:
            pscore = predictor(restaurants[i])
            dic[name] = pscore
    return dic
    # END Question 9
